...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT u.user_id, u.sex, u.birthday, u.pay_points, u.rank_points, u.user_rank , u.user_money, u.frozen_money, u.credit_line, u.parent_id, u2.user_name as parent_username, u.qq, u.msn,
    u.office_phone, u.home_phone, u.mobile_phone, u.state". /* mod by www.53moban.com 新注册会员登录审核插件 */
        " FROM " .$ecs->table('users'). " u LEFT JOIN " . $ecs->table('users') . " u2 ON u.parent_id = u2.user_id WHERE u.user_id='$_GET[id]'";
    ...
    if ($row)
    {
        ...
        $user['home_phone']     = $row['home_phone'];
        $user['mobile_phone']   = $row['mobile_phone'];
        $user['state'] = $row['state']; /* add by www.53moban.com 新注册会员登录审核插件 */
    }
    else
    {
          $link[] = array('text' => $_LANG['go_back'], 'href'=>'users.php?act=list');
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    $rank = empty($_POST['user_rank']) ? 0 : intval($_POST['user_rank']);
    $credit_line = empty($_POST['credit_line']) ? 0 : floatval($_POST['credit_line']);
    $state = empty($_POST['state']) ? 0 : intval($_POST['state']); /* add by www.53moban.com 新注册会员登录审核插件 */
    $users  =& init_users();
    ...
    $other['credit_line'] = $credit_line;
    $other['user_rank'] = $rank;
    $other['state'] = $state; /* add by www.53moban.com 新注册会员登录审核插件 */
    $other['msn'] = isset($_POST['extend_field1']) ? htmlspecialchars(trim($_POST['extend_field1'])) : '';
    $other['qq'] = isset($_POST['extend_field2']) ? htmlspecialchars(trim($_POST['extend_field2'])) : '';
    $other['office_phone'] = isset($_POST['extend_field3']) ? htmlspecialchars(trim($_POST['extend_field3'])) : '';
    $other['home_phone'] = isset($_POST['extend_field4']) ? htmlspecialchars(trim($_POST['extend_field4'])) : '';
...
function user_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT user_id, user_name, email, is_validated, user_money, frozen_money, rank_points, pay_points, reg_time, state ". /* mod by www.53moban.com 新注册会员登录审核插件 */
